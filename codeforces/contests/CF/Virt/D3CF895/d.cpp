...
void solve() {
    ...
    if (x == y) {
        ...
        ll both = floor(n / lcm(x, y)   );
        ll xgroups = floor(n / x) - both;
        ll ygroups = floor(n / y) - both;
        
        // sum from n to n - xgroups
        ll xsum = (n * (n + 1)) / 2 - ((n - xgroups) * (n - xgroups + 1) / 2);
        ll ysum = (ygroups * (ygroups + 1)) / 2;
        cout << xsum - ysum << endl;

        // cout << xgroups << " " << ygroups << endl;
    }
}
